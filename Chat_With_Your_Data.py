...
def main():
    ...
    st.write('### 😎😊Welcome to ours data analysis tool. This tool can asist your data analysis tasks. Please enjoy!')

    # Load llm model
    llm = load_llm(model_name=MODEL_NAME)
    logger.info(f"Success fully load model {MODEL_NAME}")

    # Upload file
    with st.sidebar:
        uploaded_filed = st.file_uploader(
    ...
    # Read cvs file
    if uploaded_filed is not None:
        st.session_state.df = pd.read_csv(uploaded_filed)
        st.write("Your uploaded data: ", st.session_state.df.sample(7))
